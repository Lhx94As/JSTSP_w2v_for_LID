...
class LBN_XSA(nn.Module):
    def __init__(self, input_dim=1024, mid_dim=256, feat_dim=64,
                 d_k=64, d_v=64, d_ff=2048, n_heads=8,
                 dropout=0.1, n_lang=3, max_seq_len=10000):
        super(LBN_XSA, self).__init__()
        self.input_dim = input_dim
        self.feat_dim = feat_dim
        self.mid_dim = mid_dim
        self.encoder = nn.Sequential(nn.Linear(input_dim, 512, bias=False),
                                     nn.ReLU(),
                                     nn.Linear(512, 256, bias=False),
                                     nn.ReLU(),
                                     nn.Linear(self.mid_dim, self.mid_dim))
        self.dropout = nn.Dropout(p=dropout)
        self.tdnn1 = nn.Conv1d(in_channels=self.mid_dim, out_channels=512, kernel_size=5, dilation=1)
        self.bn1 = nn.BatchNorm1d(512, momentum=0.1, affine=False)
        self.tdnn2 = nn.Conv1d(in_channels=512, out_channels=512, kernel_size=5, dilation=2)
        self.bn2 = nn.BatchNorm1d(512, momentum=0.1, affine=False)
        self.tdnn3 = nn.Conv1d(in_channels=512, out_channels=512, kernel_size=1, dilation=1)
    ...
    def forward(self, x, seq_len, atten_mask, eps=1e-5):
        ...
        x = self.encoder(x)
        x = x.view(batch_size * T_len, -1, self.mid_dim).transpose(-1, -2)
        x = self.bn1(F.relu(self.tdnn1(x)))
        x = self.bn2(F.relu(self.tdnn2(x)))
        x = self.bn3(F.relu(self.tdnn3(x)))
