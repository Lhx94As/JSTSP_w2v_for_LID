...
class X_Transformer_E2E_LID(nn.Module):
    ...
    def forward(self, x, seq_len, mean_mask_, weight_mean, std_mask_, weight_unbaised, atten_mask=None, eps=1e-5):
        batch_size = x.size(0)
        T_len = x.size(1)
        x = self.dropout(x)
        ...
        stats = self.mean_std_pooling(output, batch_size, seq_len, mean_mask_, weight_mean, std_mask_, weight_unbaised)
