...
class SE_XSA(nn.Module):
    def __init__(self, input_dim, feat_dim,
        ...
        self.input_dim = input_dim
        self.feat_dim = feat_dim
        self.SElayer = SElayer(dim=self.input_dim)
    ...
    def forward(self, x, seq_len, seq_weights, std_mask_, weight_unbaised, atten_mask=None, eps=1e-5):
        ...
        x, se_ = self.SElayer(x, seq_weights) # x: (batchsize, frames, input_dim)
...
class AE_conv_XSA(nn.Module):
    def __init__(self, input_dim=1024, middim=256, feat_dim=64,
                 d_k=64, d_v=64, d_ff=64, n_heads=4,
                 dropout=0.1, n_lang=3, max_seq_len=10000):
        super(AE_conv_XSA, self).__init__()
        self.input_dim = input_dim
        self.feat_dim = feat_dim
        self.middim = middim
        self.enc_conv = nn.Sequential(nn.Conv1d(in_channels=input_dim, out_channels=512, kernel_size=1, dilation=1),
                                      nn.ReLU(),
                                      nn.BatchNorm1d(512, momentum=0.1, affine=False),
                                      nn.Conv1d(in_channels=512, out_channels=512, kernel_size=1, dilation=1),
                                      nn.ReLU(),
                                      nn.BatchNorm1d(512, momentum=0.1, affine=False))
        self.enc_fc = nn.Sequential(nn.Linear(512, 256),
                                    nn.Tanh(),
                                    nn.Linear(256, 256))
        self.dec_fc = nn.Sequential(nn.Linear(256, 256),
                                    nn.Tanh(),
                                    nn.Linear(256, 512))
        self.dec_conv = nn.Sequential(nn.ReLU(),
                                      nn.BatchNorm1d(512, momentum=0.1, affine=False),
                                      nn.Conv1d(in_channels=512, out_channels=512, kernel_size=1, dilation=1),
                                      nn.ReLU(),
                                      nn.BatchNorm1d(512, momentum=0.1, affine=False),
                                      nn.Conv1d(in_channels=512, out_channels=input_dim, kernel_size=1, dilation=1))

        self.xvector_module = nn.Sequential(nn.Dropout(p=dropout),
                                            nn.Conv1d(in_channels=middim, out_channels=512, kernel_size=5, dilation=1),
                                            nn.ReLU(),
                                            nn.BatchNorm1d(512, momentum=0.1, affine=False),
                                            nn.Conv1d(in_channels=512, out_channels=512, kernel_size=5, dilation=2),
                                            nn.ReLU(),
    ...
    def forward(self, x, seq_len, seq_weights, std_mask_, weight_unbaised, atten_mask=None, eps=1e-5):
        ...
        x = self.enc_conv(x)
        latent_rep = self.enc_fc(x.transpose(1,2))
        x = self.dec_fc(latent_rep)
        ae_output = self.dec_conv(x.transpose(1,2))
        x = latent_rep.view(batch_size*T_len, -1, self.middim).transpose(-1, -2)
        x = self.xvector_module(x)
        if self.training:
            shape = x.size()
            noise = torch.Tensor(shape)
            noise = noise.type_as(x)
        ...
        output = F.relu(self.fc2(output))
        output = self.fc3(output)
        return output, ae_output.transpose(1,2).contiguous().view(-1, self.input_dim)


class autoencoder_LID(nn.Module):
    def __init__(self, input_dim, mid_dim):
        super(autoencoder_LID, self).__init__()
        self.input_dim = input_dim
        self.enc_conv = nn.Sequential(nn.Conv1d(in_channels=input_dim, out_channels=512, kernel_size=1, dilation=1),
                                      nn.ReLU(),
                                      nn.BatchNorm1d(512, momentum=0.1, affine=False),
        ...
        self.dec_fc = nn.Sequential(nn.Linear(mid_dim, 256),
                                    nn.Tanh(),
                                    nn.Linear(256, 512))
        self.dec_conv = nn.Sequential(nn.ReLU(),
                                      nn.BatchNorm1d(512, momentum=0.1, affine=False),
    ...
    def forward(self, x):
        ...
        x = self.dec_fc(latent_rep)
        ae_output = self.dec_conv(x.transpose(1, 2))
        return ae_output.transpose(1, 2).contiguous().view(-1, self.input_dim)

class LDA_XSA(nn.Module):
    def __init__(self, input_dim=1024, middim=256, feat_dim=64,
                 d_k=64, d_v=64, d_ff=64, n_heads=4,
                 dropout=0.1, n_lang=3, max_seq_len=10000):
        super(LDA_XSA, self).__init__()
        self.input_dim = input_dim
        self.feat_dim = feat_dim
        self.middim = middim
        self.encoder = nn.Sequential(nn.Linear(input_dim, 512),
...
class SE_LDA_XSA(nn.Module):
    def __init__(self, input_dim, feat_dim,
                 d_k, d_v, d_ff, n_heads=4,
                 dropout=0.1, n_lang=3, max_seq_len=10000):
        super(SE_LDA_XSA, self).__init__()
        self.input_dim = input_dim
        self.feat_dim = feat_dim
